    ...
    <div className="un-root">
      <header className="user-header">
        ...
        <div className="user-header__right">
          <Link to="/notifications" className="user-header__bell user-header__bell--active" aria-label="Open notifications">
      ...
      <main className="un-main">
        <section className="un-hero">
          <div>
            ...
            <p>Stay updated on approvals, rejections, cancellations, and every booking event created by the backend.</p>
          </div>
          <div className="un-hero__actions">
            <Link to="/bookings" className="un-btn un-btn--ghost">My Bookings</Link>
            <button type="button" className="un-btn un-btn--primary" onClick={loadNotifications}>Refresh</button>
          </div>
        </section>
        ...
        <section className="un-content">
          <div className="un-toolbar">
            <div>
              <h2>{session.name ? `${session.name}'s notifications` : 'Your notifications'}</h2>
          ...
          {loading ? (
            ...
            <div className="un-empty">
              ...
              <p>When bookings are approved, rejected, or cancelled, your updates will appear here.</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="un-card__body">
                    <div className="un-card__top">
                      <div>
                        <p className="un-card__label">{notification.read_status ? 'Seen' : 'New Update'}</p>
                        <h3>{notification.message}</h3>
                      </div>
                      <span className="un-card__time">{formatDate(notification.created_at)}</span>
                    </div>
                    {!notification.read_status && (
                      <div className="un-card__actions">
                        ...
                        >
                          {actingId === notification.notification_id ? 'Saving...' : 'Mark as read'}
                        </button>
                      </div>
                    )}
                  </div>
